...
if(!function_exists('split_page')){
	function  split_page($page,$who){
	   $oh=explode("<span class=\"tt-w\"><a class =\"t\" href=\"http://".$who.".deviantart.com/art/",$page);
...
switch(strtolower($kind)){
	...
	case 'random':
       ...
       while($dev>0){
	       ...
	       $me=rand(1,count($oh));
	       if(!$done[$me]){
		       if(substr($oh[$me],0,7)==":thumb:"|| substr($oh[$me],7,16)=="<!DOCTYPE:"){
